...
function solve(){
    return function (animals) {
        ...
        _.chain(groups)
        .each(function(animalsInGroup) {
            var sortedAnimals = _.chain(animalsInGroup)            
            .sortBy(function(animal) {
                return animal.name;
            })
            .sortBy(function(animal) {
            ...
            var separator = Array((animalsInGroup[0].species.length) + 2).join('-');

            console.log(separator);
            console.log(animalsInGroup[0].species + ':');
            console.log(separator);
